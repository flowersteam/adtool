...
class ExperimentPipeline(Leaf):
    ...
    def run(self, n_exploration_runs: int):
        ...
        try:
         ...
         #   print("self._explorer.parameter_map",self._explorer.parameter_map)
            # wrap sample by concatenating the output with the target

            # Store a reference to the original sample method
            original_sample = self._explorer.behavior_map.sample

            def wrap_sample():
                data_dict = original_sample()  # Call the original sample method
                dynamic_params = data_dict
                shape = dynamic_params.shape
                target = np.random.rand(*shape)
                
                # Concatenate the target to the dynamic_params
                data_dict= np.concatenate((dynamic_params, target), axis=0)
            ...
            while self.run_idx < n_exploration_runs:
                ...
                data_dict = self._explorer.map(data_dicts) 
